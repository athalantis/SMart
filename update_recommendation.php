...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['produk_id'], $_POST['recommendations'])) {
    ...
    if ($stmt->execute()) {
        echo json_encode(["success" => true, "message" => "Status rekomendasi berhasil diperbarui."]);
    } else {
        echo json_encode(["success" => false, "message" => "Gagal memperbarui status rekomendasi."]);
    }
