...
// Update the query to fix the `stok` reference
$query = "SELECT p.produk_id, p.nama_produk, p.harga, p.stok, p.gambar_produk,
          COALESCE(SUM(pm.jumlah), 0) as total_pembelian
          FROM produk p
          LEFT JOIN pemesanan pm ON p.produk_id = pm.produk_id
          WHERE p.nama_produk LIKE '%$search%'";

if ($filter == 'most_purchased') {
    $query .= " GROUP BY p.produk_id ORDER BY total_pembelian DESC";
} elseif ($filter == 'most_stock') {
    $query .= " GROUP BY p.produk_id ORDER BY p.stok DESC";
} else {
    $query .= " GROUP BY p.produk_id ORDER BY p.produk_id DESC";
}
