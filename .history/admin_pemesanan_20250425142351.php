...
// Handle request AJAX untuk update rekomendasi
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_recommendation'])) {
    $produk_id = $_POST['produk_id'];
    $recommendations = $_POST['recommendations'] === '1' ? 1 : 0;

    $stmt = $conn->prepare("UPDATE produk SET recommendations = ? WHERE produk_id = ?");
    $stmt->bind_param("ii", $recommendations, $produk_id);
    $stmt->execute();
    $stmt->close();
    exit(); // Stop output karena ini hanya untuk AJAX
}
                ...
                <tbody>
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<td>
                                    <input type='checkbox' class='recommend-checkbox' data-id='{$row['produk_id']}' " . ($row['recommendations'] ? 'checked' : '') . ">
                                  </td>";
                            echo "<td>
                                    <form method='post'>
                        ...
                        echo "<tr><td colspan='10' class='text-center'>Belum ada pemesanan</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
...
<!-- Inline AJAX Script -->
<script>
document.querySelectorAll('.recommend-checkbox').forEach(checkbox => {
    checkbox.addEventListener('change', function () {
        const produkId = this.dataset.id;
        const isChecked = this.checked ? 1 : 0;

        const xhr = new XMLHttpRequest();
        xhr.open("POST", "", true); // POST ke halaman ini
        xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhr.send("update_recommendation=1&produk_id=" + produkId + "&recommendations=" + isChecked);
    });
});
</script>
