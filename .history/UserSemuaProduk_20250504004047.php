...
if (isset($_GET['action']) && $_GET['action'] === 'fetch_products') {
    ...
    // Generate HTML for products (using existing card structure assumed to be handled elsewhere)
    $output = '';
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            // Assuming the card structure is handled by the frontend or another script
...
<script>
    ...
    $(document).ready(function () {
        ...
        function fetchProducts() {
            ...
            $.ajax({
                url: window.location.href, // Use current URL to avoid 404
                type: 'GET',
                data: {
                ...
                success: function(response) {
                    ...
                    // Dynamically attach modal triggers to new elements
                    $('#product-list [data-produk-id]').each(function() {
                        var $this = $(this);
                        $this.replaceWith(`
                            <div class="col-sm-5 col-md-3 me-3 mb-4">
                                <div class="card h-100 hovered-card">
                                    <a href="#">
                                        <img class="card-img-top" src="./product_picture/' . htmlspecialchars($row['gambar_produk']) . '" alt="' . htmlspecialchars($row['nama_produk']) . '" style="height: 200px; object-fit: cover; border-top-left-radius: 0.5rem; border-top-right-radius: 0.5rem;" />
                                        ...
                                        <div class="card-body">
                                            ...
                                            <button type="button" class="btn rounded-pill btn-primary w-100" data-bs-toggle="modal" data-bs-target="#orderModal" onclick="setOrderModal(' . $row['produk_id'] . ', ' . $row['stok'] . ', \'' . htmlspecialchars($row['nama_produk']) . '\', ' . $row['harga'] . ')">
                                                Beli Sekarang
                                            </button>
                                        </div>
                                    </a>
                        ...
                        `.replace(/'\s*\.\s*htmlspecialchars\(/g, '').replace(/\)\s*\.\s*'/g, ''));
                    });
                },
