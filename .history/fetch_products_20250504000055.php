...
// Logic for handling order placement (can be moved to a separate file)
if (isset($_POST['order'])) {
    $produk_id = $_POST['produk_id'];
    $user_id = $_POST['user_id'];  // You may need to retrieve the user_id from session or authentication system
    $tanggal_pemesanan = date('Y-m-d');
    $jumlah_pemesanan = $_POST['jumlah_pemesanan'];
    $status_pemesanan = 'pending';  // Default status

    // Check if there's enough stock
    $stockQuery = "SELECT stok FROM produk WHERE produk_id = $produk_id";
    $stockResult = mysqli_query($conn, $stockQuery);
    $stock = mysqli_fetch_assoc($stockResult)['stok'];

    if ($stock >= $jumlah_pemesanan) {
        // Insert order into 'pemesanan' table
        $orderQuery = "INSERT INTO pemesanan (produk_id, user_id, tanggal_pemesanan, jumlah_pemesanan, status_pemesanan) 
                       VALUES ($produk_id, $user_id, '$tanggal_pemesanan', $jumlah_pemesanan, '$status_pemesanan')";
        if (mysqli_query($conn, $orderQuery)) {
            // Optionally, reduce the stock
            $newStock = $stock - $jumlah_pemesanan;
            $updateStockQuery = "UPDATE produk SET stok = $newStock WHERE produk_id = $produk_id";
