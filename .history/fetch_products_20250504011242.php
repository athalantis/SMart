...
// Handle AJAX order submission first to avoid HTML output interference
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'place_order') {
    ...
    if (!$stock_row || $stock_row['stok'] <= 0 || $stock_row['stok'] < $jumlah_pemesanan) {
...
// Ensure user is authenticated for the product list
if (!isset($_SESSION['user_id'])) {
...
?>
                        ...
                        <div class="card h-100 hovered-card">
                            <a href="#">
                                ...
                                <div class="card-body">
                                    ...
                                    <p class="card-location">🚚 <?php echo $stok > 0 ? 'Tersedia' : 'Habis'; ?></p>
                                    <p class="card-rating">⭐ 4.9 | Stok: <?php echo $stok; ?> tersedia</p>
                                    <button type="button" 
                                            class="btn rounded-pill btn-primary w-100 <?php echo $stok <= 0 ? 'disabled' : ''; ?>" 
                                            data-bs-toggle="modal" 
                                            data-bs-target="#orderModal"
                                            onclick="<?php echo $stok > 0 ? "setOrderModal({$row['produk_id']}, {$stok}, '" . htmlspecialchars(str_replace("'", "\'", $row['nama_produk'])) . "')" : ''; ?>">
                                        Beli Sekarang
                                    </button>
                                </div>
                            </a>
    ...
    <script>
        ...
        function setOrderModal(produk_id, stok, nama_produk) {
            ...
            document.getElementById('jumlah_pemesanan').value = 1; // Reset quantity to 1
            maxStock = stok;
        }

        function submitOrder() {
    const produk_id = document.getElementById('produk_id').value;
    const jumlah_pemesanan = parseInt(document.getElementById('jumlah_pemesanan').value);

    if (!jumlah_pemesanan || jumlah_pemesanan <= 0 || maxStock <= 0 || jumlah_pemesanan > maxStock) {
