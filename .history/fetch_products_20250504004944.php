...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'place_order') {
        ...
        // Verify stock
        $stock_query = "SELECT stok FROM produk WHERE produk_id = $produk_id";
        $stock_result = mysqli_query($conn, $stock_query);
        $stock_row = mysqli_fetch_assoc($stock_result);

        if ($stock_row['stok'] < $jumlah_pemesanan) {
        ...
        if (mysqli_query($conn, $insert_query)) {
            // Update stock
            $update_stock = "UPDATE produk SET stok = stok - $jumlah_pemesanan WHERE produk_id = $produk_id";
